...
boot_alloc(uint32_t n)
{
	...
	if ((uint32_t)(PADDR(nextfree)+n) > npages*PGSIZE)
...
mem_init_mp(void)
{
	...
	// NOTE: Modified to support my changes for PSE from lab 2
	if (cpu_has_pse())
	{
		boot_map_region_pse(kern_pgdir, IOMEMBASE, -IOMEMBASE - PGSIZE, IOMEM_PADDR, PTE_W);
	}
	else
	{
		boot_map_region(kern_pgdir, IOMEMBASE, -IOMEMBASE - PGSIZE, IOMEM_PADDR, PTE_W);
	}
	...
	for (i = 0; i < NCPU; i++)
	{
		...
		{
			struct Page *page = pa2page(PADDR(percpu_kstacks[i])+j);
			if (!page)
				panic("Out of memory for CPU %d kstack\n", i);
			if (page_insert(kern_pgdir, page, 
							(void*)(kstacktop_i-KSTKSIZE + j), PTE_W) < 0)
				panic("Failed to allocate page table for CPU %d kstack alloc\n", i);
		}
		// Remove mappings for guard pages if they exist
		void *va;
		for (va = (void*)kstacktop_i-(KSTKSIZE + KSTKGAP); 
					va < (void*)kstacktop_i-KSTKSIZE; va += PGSIZE)
		{
			page_remove(kern_pgdir, va);
		}
	}
...
page_init(void)
{
	...
	page_free_list = 0; // Initialize to NULL

	size_t i;

	for (i = 0; i < npages; i++) {

		// Recognize the four holes:
		// 1. Page 0
		// 2. Pages in [IOPHYSMEM, EXTPHYSMEM)
		// 3. Pages in [EXTPHYSMEM, end of "pages" array)
		// 4. (NEW) Page at MPENTRY_PADDR
		...
		{
			// Set the ref to 1 for "in use," set link to NULL
			pages[i].pp_ref = 1;
			pages[i].pp_link = 0;
		}
		else 
...
page_alloc(int alloc_flags)
{
	...
	// if the free list is NULL
	if (page_free_list == NULL)
		return NULL;
	
	// The page we will return is the head
	// of the free list
...
{
	// If the PRESENT bit is not set and
	// create is set, construct a new page table
	// and put the entry in the page directory
	...
	{
		// Find the address of the page table
		pte_t *pgtab = KADDR(PTE_ADDR(pgdir[PDX(va)]));

		// Get the address of the correct entry
		return &pgtab[PTX(va)];
	}
	else
	{
...
boot_map_region_pse(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	...
	cprintf("PSE MAPPING [0x%08x, 0x%08x] to [0x%08x, 0x%08x]\n", pa, ROUNDUP(pa+size, PGSIZE*1024)-1, va, ROUNDUP(va+size, PGSIZE*1024)-1); 
...
{
	// Find the page table entry, store it if necessary,
	// if it is mapped return the struct Page
	// associated with that physical address.
	...
	// return NULL if the page table does not exist
	// or if the entry is not present
	if ((!pte) || !(*pte & PTE_P))
	{
		return NULL;
	}
