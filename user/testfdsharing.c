...
umain(int argc, char **argv)
{
	...
	if (r == 0) {
		seek(fd, 0);
		printf("going to write in child\n");
		if ((n = write(fd, "world", 5)) != 5)
			panic("write in child: %e", n);
		printf("write in child finished\n");
		close(fd);
		exit();
	}
	wait(r);
	...
	if (strcmp(buf, "hello") == 0)
		printf("write to file failed; got old data\n");
	else if (strcmp(buf, "world") == 0)
		printf("write to file succeeded\n");
	else
		printf("write to file failed; got %s\n", buf);
